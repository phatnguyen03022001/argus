...
For Neon observation, an active environment profile must contain the ordinary setting `NEON_PROJECT_ID` and the credential binding `NEON_API_KEY`. An operator-triggered refresh performs one bounded `GET /api/v2/projects/{project_id}` using the Keychain-resolved API key only at the HTTP boundary, persists only normalized safe project metadata, and retains the last known good observation as `STALE` when a later refresh cannot establish current truth. Argus does not list Neon resources, persist connection strings or raw provider responses, mutate Neon state, retry, poll, watch, or run background refreshes.
...
Argus currently includes the Phase 2 workspace foundation, Phase 3 read-only repository/worktree discovery with Git/GitHub observation, Phase 4 credential-reference/Keychain plus workspace environment-profile configuration boundaries, and the bounded Phase 5 read-only Neon project observation slice. Phase 3 derives repository health, sync condition, attention, and reason codes from current observations without persisting them as source truth; it does not perform Git mutation or sync actions. Phase 4 stores credential references plus explicit workspace-scoped environment names, ordinary non-secret scalar settings, and credential-reference bindings; Keychain availability/resolution remains read-only and scoped to an in-process consumer. Argus does not create, rotate, revoke, copy, reveal, or persist credential values; it does not infer environments from Git branches, materialize `.env` files, apply/sync environment configuration, or implement provider/OAuth/cloud-vault integration. Task projection, agent execution, business capabilities, generic provider/resource management, deployment mapping, and later Phase 5+ surfaces remain out of scope.
